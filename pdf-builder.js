...
(function () {
    ...
    function buildTheory() {
        ...
        t = t.replace(/<div class="theory-box"\s+style="background:\s*#F5F0E8[^"]*"[^>]*>/g, '<div class="theory-box warn-box">');
        return `<h1 class="chapter">📘 1. Teorie — In Mihais Küche (vocabular bucătărie)</h1>` + t;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillInBlock('Übung 1 — Verpackungen: ambalaje și cantități (eine Flasche, ein Glas...)', 'Cele mai folosite: eine Flasche (sticlă), ein Glas (borcan), eine Dose (cutie), eine Packung (pachet), eine Tafel (tabletă de ciocolată), eine Tasse (cană), eine Tüte (pungă).', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillInBlock('Übung 2 — Hauptgerichte și ingrediente (carne, legume, garnituri)', 'Carne: Schweinefleisch, Rindfleisch, Hühnchen, Wurst · Garnituri: Kartoffeln, Reis, Nudeln · Legume: Tomaten, Gurken, Salat.', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillInBlock('Übung 3 — Suppen, Vorspeisen și forme de servire', 'Suppen: Gemüsesuppe, Hühnersuppe, Tomatensuppe · Vorspeisen: Salat, Brot, Käseplatte.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillInBlock('Übung 4 — Verbe de gătit (kochen, backen, braten, schneiden...)', 'kochen (a găti/fierbe) · backen (a coace) · braten (a prăji) · schneiden (a tăia) · schälen (a curăța de coajă) · würzen (a condimenta).', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += fillInBlock('Übung 5 — Küchengeräte (cuțit, furculiță, tigaie, oală...)', 'das Messer (cuțitul), die Gabel (furculița), der Löffel (lingura), die Pfanne (tigaia), der Topf (oala), der Teller (farfuria).', ex5Data);
        }
        return html;
    }
